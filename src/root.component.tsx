...
export default function Root(props) {
  ...
  useEffect(() => {
    document.addEventListener("countEvent", listenerEvent);
    document.addEventListener("changeState:user", resetStateUser);

    return () => {
      document.removeEventListener("countEvent", listenerEvent);
      document.removeEventListener("changeState:user", resetStateUser);
    };
  }, []);
  ...
  const handleRandom = () => {
    ...
    document.dispatchEvent(
      new CustomEvent("changeState:user", { detail: { origin: "react", user } })
    );
  };

  const resetStateUser = (event) => {
    if (event.detail.origin === "react") return;
  ...
  return (
    <section>
      ...
      <h1>Zustand</h1>
      <pre style={{ marginBottom: 20 }}>{JSON.stringify(user, null, 2)}</pre>
      <button onClick={handleRandom}>Alterar</button>
    </section>
  );
}
